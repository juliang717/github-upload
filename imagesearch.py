...
def imagesearcharea(image, x1, y1, x2, y2, precision=0.8, im=None, absolute=False):
    if im is None:
    ...
    if absolute:
        return_pos = [max_loc[0] + x1, max_loc[1] + y1]
        return return_pos
    return max_loc
...
def imagesearch_region_loop(image, timesample, x1, y1, x2, y2, precision=0.8, absolute=False):
    pos = imagesearcharea(image, x1, y1, x2, y2, precision, absolute=absolute)
